...
namespace cadm {
    ...
    struct vec_base {
        ...
        friend constexpr bool operator!=(const Derived &lhs, const Derived &rhs) noexcept { return !(lhs == rhs); }

        constexpr Derived& operator+=(const Derived &rhs) {
            for (int i = 0; i < N; ++i) { (*this)[i] += rhs.data[i]; }
        ...
        void normalize() noexcept {
            ...
            if (std::abs(lengthSq - static_cast<T>(1)) < eps) { return; }
        ...
        [[nodiscard]] Derived safeNormalized(const Derived &fallback) const noexcept {
            const auto lengthSq = lengthSquared();
            if (lengthSq < eps * eps) { return fallback; }
        ...
        constexpr T lengthSquared() const noexcept {
            T res{};
            for (int i = 0; i < N; ++i) { res += (*this)[i] * (*this)[i]; }
            return res;
        }

        constexpr T dot(const Derived &other) const noexcept {
            T res = 0;
            for (int i = 0; i < N; ++i) { res += other.data[i] * (*this)[i]; }
            return res;
        }
    };
}
